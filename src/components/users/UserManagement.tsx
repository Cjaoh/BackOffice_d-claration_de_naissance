    ...
    <div className="space-y-6">
      <div className="bg-white shadow rounded-lg p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
          ...
          <Button onClick={handleCreateUser} className="bg-[#4CAF9E] hover:bg-[#3d8b7f] text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div>
            <Input type="text" placeholder="Rechercher par nom ou email..." value={searchTerm} onChange={handleSearchChange} />
          </div>
          <div>
            <select
                  ...
                  <tr key={user.uid} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          <div className="text-sm font-medium text-gray-900">{user.displayName || "Anonyme"}</div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button onClick={() => handleEditUser(user)} className="text-[#4CAF9E] hover:text-[#3d8b7f] mr-3">Modifier</button>
                      <button onClick={() => handleDeleteUser(user.uid)} className="text-red-600 hover:text-red-900">Supprimer</button>
                    </td>
                  </tr>
                ))
              )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <div className="space-y-4">
              <Input label="Nom" type="text" placeholder="Nom complet" value={newUser.displayName} onChange={(value) => handleNewUserChange('displayName', value)} />
              <Input label="Email" type="email" placeholder="Adresse email" value={newUser.email} onChange={(value) => handleNewUserChange('email', value)} />
              <Input label="Mot de passe" type="password" placeholder="Mot de passe" value={newUser.password} onChange={(value) => handleNewUserChange('password', value)} />
              <div>
            ...
            <div className="mt-6 flex justify-end space-x-3">
              <Button onClick={() => setShowCreateModal(false)} className="bg-gray-300 hover:bg-gray-400 text-gray-700">Annuler</Button>
              <Button onClick={handleCreateUserSubmit} className="bg-[#4CAF9E] hover:bg-[#3d8b7f] text-white">Créer</Button>
            </div>
          </div>
        </div>
      )}
      {showEditModal && editingUser && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <div className="space-y-4">
              <Input label="Nom" type="text" value={editingUser.displayName} placeholder="Nom complet" onChange={(value: string) => handleEditingUserChange('displayName', value)} />
              <Input label="Email" type="email" value={editingUser.email} placeholder="Adresse email" onChange={(value: string) => handleEditingUserChange('email', value)} />
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Rôle</label>
                <select
                  className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#4CAF9E] focus:border-[#4CAF9E]"
            ...
            <div className="mt-6 flex justify-end space-x-3">
              <Button onClick={() => setShowEditModal(false)} className="bg-gray-300 hover:bg-gray-400 text-gray-700">Annuler</Button>
              <Button onClick={handleEditUserSubmit} className="bg-[#4CAF9E] hover:bg-[#3d8b7f] text-white">Enregistrer</Button>
            </div>
          </div>
        </div>
      )}
